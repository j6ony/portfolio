...
const BorderGlow = ({
  children, className = '',
  edgeSensitivity = 30, glowColor = '40 80 80',
  backgroundColor = '#120F17', borderRadius = 28,
  glowRadius = 40, glowIntensity = 1.0, coneSpread = 25,
  animated = false, fillOpacity = 0.5,
  ...
  const getEdgeProximity = useCallback((el, x, y) => {
    ...
    return Math.max(0.15, Math.min(Math.max(1 / Math.min(kx, ky), 0), 1));
  }, [getCenterOfElement]);
  ...
  useEffect(() => {
    ...
    const card = cardRef.current; const angleStart = 110; const angleEnd = 465;
  ...
  return (
    <div ref={cardRef} onPointerMove={handlePointerMove} className={`border-glow-card ${className}`} style={{
      ...
      ...glowVars, ...buildGradientVars(colors),
    }}>
