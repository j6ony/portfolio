...
const Particles = ({
  ...
  moveParticlesOnHover = false, particleHoverFactor = 1,
  ...
  useEffect(() => {
    ...
    if (moveParticlesOnHover) window.addEventListener('mousemove', onMouseMove, { passive: true });
    ...
    let lastTime = performance.now(), elapsed = 0, animId;
    const update = t => {
      animId = requestAnimationFrame(update);
      const delta = t - lastTime; lastTime = t; elapsed += delta * speed;
      program.uniforms.uTime.value = elapsed * 0.001;
      if (moveParticlesOnHover) { particles.position.x = -mouseRef.current.x * particleHoverFactor; particles.position.y = -mouseRef.current.y * particleHoverFactor; }
